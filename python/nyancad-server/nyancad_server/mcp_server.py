...
async def bulk_update_schematic(
    ...
    3. Strips `_computed` fields if present (auto-regenerated by server after update)
    4. Submits merged documents via CouchDB _bulk_docs

    **Note:** Never include `_computed` in your updates - it's automatically calculated
    from device geometry and will be regenerated after the update completes.

    **Avoiding complex wire routing:**
    Instead of creating multiple wire segments, use port labels for cleaner schematics.
    Add a port device with a name, and any other port with the same name creates an
    implicit connection without drawing wires.

    Args:
        ctx: MCP context
        schematic_id: Schematic group name (bare ID without document prefix)
        docs: List of device updates. Each must have _id. Include _rev for updates,
              omit for new documents. Set _deleted=true to delete.
